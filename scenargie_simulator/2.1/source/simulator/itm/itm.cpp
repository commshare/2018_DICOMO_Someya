...
//ITMDLL.cpp -> itm.cpp
#include "itm.h" //ScenSim-Port://

namespace ITM {//ScenSim-Port://

#define THIRD  (1.0/3.0)
//ScenSim-Port://#define DllExport  __declspec( dllexport )
#define DllExport  //ScenSim-Port
#define _stdcall  //ScenSim-Port
  ...
  if(prop.mdp!=0)
    {
      ...
      for(j=0;j<2;j++)
        //ScenSim-Port:// if( abs(prop.the[j]) >200e-3 || prop.dl[j]<0.1*propa.dls[j] ||
        if( fabs(prop.the[j]) >200e-3 || prop.dl[j]<0.1*propa.dls[j] ||
           prop.dl[j]>3.0*propa.dls[j] )
        { prop.kwx=mymax(prop.kwx,3);
        }
      if( prop.ens < 250.0   || prop.ens > 400.0  ||
          prop.gme < 75e-9 || prop.gme > 250e-9 ||
          //ScenSim-Port:// prop_zgnd.real() <= abs(prop_zgnd.imag()) ||
          prop_zgnd.real() <= fabs(prop_zgnd.imag()) ||
      ...
      //ScenSim-Port:// dmin=abs(prop.he[0]-prop.he[1])/200e-3;
...
{ int np, ka, kb, n, k, j;
  ...
  s = new double[n + 2];
  assert(s != nullptr);
  //Crash! assert( (s = new double[n+2]) != 0 );
  s[0]=sn;
  s[1]=1.0;
  xb=(xb-xa)/sn;
  k=(int)(xa+1.0);
...
void DllExport _stdcall point_to_point(double elev[], double tht_m, double rht_m,
          ...
          double &dbloss/*//ScenSim-Port://, char *strmode*/, int &errnum)
    ...
    //                          Results are probably invalid.
{
  ...
  if(q<=0.0)
  {
    //ScenSim-Port:// ja = 3.0 + 0.1 * elev[0];
    ja = static_cast<long>(3.0 + 0.1 * elev[0]); //ScenSim-Port://
    jb = np - ja + 6;
    for(i=ja-1;i<jb;++i)
      zsys+=elev[i];
    zsys/=(jb-ja+1);
  ...
  //ScenSim-Port:// if(int(q)<0.0)
  //ScenSim-Port://   strcpy(strmode,"Line-Of-Sight Mode");
  //ScenSim-Port:// else
  //ScenSim-Port://   { if(int(q)==0.0)
  //ScenSim-Port://       strcpy(strmode,"Single Horizon");
  //ScenSim-Port://     else if(int(q)>0.0)
  //ScenSim-Port://       strcpy(strmode,"Double Horizon");
  //ScenSim-Port://     if(prop.dist<=propa.dlsa || prop.dist <= propa.dx)
  //ScenSim-Port://       strcat(strmode,", Diffraction Dominant");
  //ScenSim-Port://     else if(prop.dist>propa.dx)
  //ScenSim-Port://       strcat(strmode, ", Troposcatter Dominant");
  //ScenSim-Port://   }
...
#undef DllExport  //ScenSim-Port
#undef _stdcall  //ScenSim-Port
