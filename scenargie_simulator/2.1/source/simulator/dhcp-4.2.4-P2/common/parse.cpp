...
//ScenSim-Port//struct enumeration *enumerations;
...
//ScenSim-Port//void skip_to_semi (cfile)
//ScenSim-Port//	struct parse *cfile;
void skip_to_semi (struct parse *cfile)//ScenSim-Port//
{
	skip_to_rbrace (cfile, 0);
}

//ScenSim-Port//void skip_to_rbrace (cfile, brace_count)
//ScenSim-Port//	struct parse *cfile;
//ScenSim-Port//	int brace_count;
void skip_to_rbrace (struct parse *cfile, int brace_count)//ScenSim-Port//
{
...
//ScenSim-Port//int parse_semi (cfile)
//ScenSim-Port//	struct parse *cfile;
int parse_semi (struct parse *cfile)//ScenSim-Port//
{
	enum dhcp_token token;
	const char *val;
...
//ScenSim-Port//int parse_string (cfile, sptr, lptr)
//ScenSim-Port//	struct parse *cfile;
//ScenSim-Port//	char **sptr;
//ScenSim-Port//	unsigned *lptr;
int parse_string (struct parse *cfile, char **sptr, unsigned *lptr)//ScenSim-Port//
{
	const char *val;
	enum dhcp_token token;
	char *s;
...
//ScenSim-Port//char *parse_host_name (cfile)
//ScenSim-Port//	struct parse *cfile;
char *parse_host_name (struct parse *cfile)//ScenSim-Port//
{
	const char *val;
	enum dhcp_token token;
	unsigned len = 0;
...
//ScenSim-Port//int parse_ip_addr_or_hostname (expr, cfile, uniform)
//ScenSim-Port//	struct expression **expr;
//ScenSim-Port//	struct parse *cfile;
//ScenSim-Port//	int uniform;
int parse_ip_addr_or_hostname (struct expression **expr, struct parse *cfile, int uniform)//ScenSim-Port//
{
	const char *val;
	enum dhcp_token token;
...
//ScenSim-Port//int parse_ip_addr (cfile, addr)
//ScenSim-Port//	struct parse *cfile;
//ScenSim-Port//	struct iaddr *addr;
int parse_ip_addr (struct parse *cfile, struct iaddr *addr)//ScenSim-Port//
{
	addr -> len = 4;
...
//ScenSim-Port//int
//ScenSim-Port//parse_ip_addr_with_subnet(cfile, match)
//ScenSim-Port//	struct parse *cfile;
//ScenSim-Port//	struct iaddrmatch *match;
int parse_ip_addr_with_subnet(struct parse *cfile, struct iaddrmatch *match)//ScenSim-Port//
{
	const char *val, *orig;
	enum dhcp_token token;
	int prefixlen;
...
//ScenSim-Port//void parse_hardware_param (cfile, hardware)
//ScenSim-Port//	struct parse *cfile;
//ScenSim-Port//	struct hardware *hardware;
void parse_hardware_param (struct parse *cfile, struct hardware *hardware)//ScenSim-Port//
{
	const char *val;
	enum dhcp_token token;
	unsigned hlen;
...
//ScenSim-Port//void parse_lease_time (cfile, timep)
//ScenSim-Port//	struct parse *cfile;
//ScenSim-Port//	TIME *timep;
void parse_lease_time (struct parse *cfile, TIME *timep)//ScenSim-Port//
{
	const char *val;
	enum dhcp_token token;
	u_int32_t num;
...
//ScenSim-Port//unsigned char *parse_numeric_aggregate (cfile, buf,
//ScenSim-Port//					max, separator, base, size)
//ScenSim-Port//	struct parse *cfile;
//ScenSim-Port//	unsigned char *buf;
//ScenSim-Port//	unsigned *max;
//ScenSim-Port//	int separator;
//ScenSim-Port//	int base;
//ScenSim-Port//	unsigned size;
unsigned char *parse_numeric_aggregate (struct parse *cfile, unsigned char *buf, unsigned *max, int separator, int base, unsigned size)//ScenSim-Port//
{
	const char *val;
	enum dhcp_token token;
...
//ScenSim-Port//void convert_num (cfile, buf, str, base, size)
//ScenSim-Port//	struct parse *cfile;
//ScenSim-Port//	unsigned char *buf;
//ScenSim-Port//	const char *str;
//ScenSim-Port//	int base;
//ScenSim-Port//	unsigned size;
void convert_num (struct parse *cfile, unsigned char *buf, const char *str, int base, unsigned size)//ScenSim-Port//
{
 ...
 */
TIME 
//ScenSim-Port//parse_date_core(cfile)
//ScenSim-Port//	struct parse *cfile;
parse_date_core(struct parse *cfile)//ScenSim-Port//
{
	int guess;
...
//ScenSim-Port//TIME 
//ScenSim-Port//parse_date(cfile)
//ScenSim-Port//       struct parse *cfile;
TIME parse_date(struct parse *cfile)//ScenSim-Port//
{
       TIME guess;
...
//ScenSim-Port//parse_option_name (cfile, allocate, known, opt)
//ScenSim-Port//	struct parse *cfile;
//ScenSim-Port//	int allocate;
//ScenSim-Port//	int *known;
//ScenSim-Port//	struct option **opt;
isc_result_t parse_option_name (struct parse *cfile, int allocate, int *known, struct option **opt)//ScenSim-Port//
{
	const char *val;
	enum dhcp_token token;
	char *uname;
...
//ScenSim-Port//	uname = dmalloc (strlen (val) + 1, MDL);
	uname = (char*)dmalloc (strlen (val) + 1, MDL);//ScenSim-Port//
	if (!uname)
...
//ScenSim-Port//void parse_option_space_decl (cfile)
//ScenSim-Port//	struct parse *cfile;
void parse_option_space_decl (struct parse *cfile)//ScenSim-Port//
{
	int token;
	const char *val;
...
//ScenSim-Port//	nu_name = dmalloc (strlen (val) + 1, MDL);
	nu_name = (char*)dmalloc (strlen (val) + 1, MDL);//ScenSim-Port//
	if (!nu_name)
...
//ScenSim-Port//	nu -> index = universe_count++;
	nu -> index = universe_count_++;//ScenSim-Port//
	if (nu -> index >= universe_max) {
//ScenSim-Port//		ua = dmalloc (universe_max * 2 * sizeof *ua, MDL);
		ua = (universe**)dmalloc (universe_max * 2 * sizeof *ua, MDL);//ScenSim-Port//
		if (!ua)
			log_fatal ("No memory to expand option space array.");
//ScenSim-Port//		memcpy (ua, universes, universe_max * sizeof *ua);
		memcpy (ua, universes_, universe_max * sizeof *ua);//ScenSim-Port//
		universe_max *= 2;
//ScenSim-Port//		dfree (universes, MDL);
		dfree (universes_, MDL);//ScenSim-Port//
//ScenSim-Port//		universes = ua;
		universes_ = ua;//ScenSim-Port//
	}
//ScenSim-Port//	universes [nu -> index] = nu;
	universes_ [nu -> index] = nu;//ScenSim-Port//
...
//ScenSim-Port//int parse_option_code_definition (cfile, option)
//ScenSim-Port//	struct parse *cfile;
//ScenSim-Port//	struct option *option;
int parse_option_code_definition (struct parse *cfile, struct option *option)//ScenSim-Port//
{
	const char *val;
	enum dhcp_token token;
...
//ScenSim-Port//	s = dmalloc(tokix + (arrayp ? 1 : 0) + 1, MDL);
	s = (char*)dmalloc(tokix + (arrayp ? 1 : 0) + 1, MDL);//ScenSim-Port//
	if (s == NULL) {
...
//ScenSim-Port//int parse_base64 (data, cfile)
//ScenSim-Port//	struct data_string *data;
//ScenSim-Port//	struct parse *cfile;
int parse_base64 (struct data_string *data, struct parse *cfile)//ScenSim-Port//
{
	enum dhcp_token token;
	const char *val;
	int i, j, k;
...
//ScenSim-Port//		t = dmalloc (l + sizeof *t, MDL);
		t = (string_list*)dmalloc (l + sizeof *t, MDL);//ScenSim-Port//
		if (!t)
...
//ScenSim-Port//int parse_cshl (data, cfile)
//ScenSim-Port//	struct data_string *data;
//ScenSim-Port//	struct parse *cfile;
int parse_cshl (struct data_string *data, struct parse *cfile)//ScenSim-Port//
{
	u_int8_t ibuf [128];
	unsigned ilen = 0;
	unsigned tlen = 0;
...
//ScenSim-Port//				dmalloc (ilen - 1 +
				dmalloc (ilen - 1 +//ScenSim-Port//
...
//ScenSim-Port//int parse_executable_statements (statements, cfile, lose, case_context)
//ScenSim-Port//	struct executable_statement **statements;
//ScenSim-Port//	struct parse *cfile;
//ScenSim-Port//	int *lose;
//ScenSim-Port//	enum expression_context case_context;
int parse_executable_statements (struct executable_statement **statements, struct parse *cfile, int *lose, enum expression_context case_context)//ScenSim-Port//
{
...
//ScenSim-Port//int parse_executable_statement (result, cfile, lose, case_context)
//ScenSim-Port//	struct executable_statement **result;
//ScenSim-Port//	struct parse *cfile;
//ScenSim-Port//	int *lose;
//ScenSim-Port//	enum expression_context case_context;
int parse_executable_statement (struct executable_statement **result, struct parse *cfile, int *lose, enum expression_context case_context)//ScenSim-Port//
{
	...
	enum dhcp_token token;
	const char *val;
//ScenSim-Port//	struct class *cta;
	struct class_type *cta;//ScenSim-Port//
...
//ScenSim-Port//		cta = (struct class *)0;
		cta = (struct class_type *)0;//ScenSim-Port//
//ScenSim-Port//		status = find_class (&cta, val, MDL);
		status = curctx->find_class (&cta, val, MDL);//ScenSim-Port//
		if (status != ISC_R_SUCCESS) {
//ScenSim-Port//			parse_warn (cfile, "class %s: %s",
//ScenSim-Port//				    val, isc_result_totext (status));
			parse_warn (cfile, "class %s: %i", val, status);//ScenSim-Port//
			skip_to_semi (cfile);
			*lose = 1;
			return 0;
		}
...
//ScenSim-Port//		if (!parse_allow_deny (&cache, cfile, flag))
		if (!curctx->parse_allow_deny (&cache, cfile, flag))//ScenSim-Port//
			return 0;
		if (!executable_statement_allocate (result, MDL))
			log_fatal ("no memory for new statement.");
...
//ScenSim-Port//		(*result) -> data.set.name = dmalloc (strlen (val) + 1, MDL);
		(*result) -> data.set.name = (char*)dmalloc (strlen (val) + 1, MDL);//ScenSim-Port//
...
//ScenSim-Port//		(*result) -> data.unset = dmalloc (strlen (val) + 1, MDL);
		(*result) -> data.unset = (char*)dmalloc (strlen (val) + 1, MDL);//ScenSim-Port//
...
//ScenSim-Port//		(*result)->data.execute.command = dmalloc(len + 1, MDL);
		(*result)->data.execute.command = (char*)dmalloc(len + 1, MDL);//ScenSim-Port//
		...
		*lose = 1;
		...
		/* Not really a statement, but we parse it here anyway
		   ...
		   parsers. */
	      case ZONE:
//ScenSim-Port//		token = next_token (&val, (unsigned *)0, cfile);
//ScenSim-Port//		zone = (struct dns_zone *)0;
//ScenSim-Port//		if (!dns_zone_allocate (&zone, MDL))
//ScenSim-Port//			log_fatal ("no memory for new zone.");
//ScenSim-Port//		zone -> name = parse_host_name (cfile);
//ScenSim-Port//		if (!zone -> name) {
//ScenSim-Port//			parse_warn (cfile, "expecting hostname.");
//ScenSim-Port//		      badzone:
//ScenSim-Port//			*lose = 1;
//ScenSim-Port//			skip_to_semi (cfile);
//ScenSim-Port//			dns_zone_dereference (&zone, MDL);
//ScenSim-Port//			return 0;
//ScenSim-Port//		}
//ScenSim-Port//		i = strlen (zone -> name);
//ScenSim-Port//		if (zone -> name [i - 1] != '.') {
//ScenSim-Port//			s = dmalloc ((unsigned)i + 2, MDL);
//ScenSim-Port//			if (!s) {
//ScenSim-Port//				parse_warn (cfile, "no trailing '.' on zone");
//ScenSim-Port//				goto badzone;
//ScenSim-Port//			}
//ScenSim-Port//			strcpy (s, zone -> name);
//ScenSim-Port//			s [i] = '.';
//ScenSim-Port//			s [i + 1] = 0;
//ScenSim-Port//			dfree (zone -> name, MDL);
//ScenSim-Port//			zone -> name = s;
//ScenSim-Port//		}
//ScenSim-Port//		if (!parse_zone (zone, cfile))
//ScenSim-Port//			goto badzone;
//ScenSim-Port//		status = enter_dns_zone (zone);
//ScenSim-Port//		if (status != ISC_R_SUCCESS) {
//ScenSim-Port//			parse_warn (cfile, "dns zone key %s: %s",
//ScenSim-Port//				    zone -> name, isc_result_totext (status));
//ScenSim-Port//			dns_zone_dereference (&zone, MDL);
//ScenSim-Port//			return 0;
//ScenSim-Port//		}
//ScenSim-Port//		dns_zone_dereference (&zone, MDL);
//ScenSim-Port//		return 1;
		...
		/* Also not really a statement, but same idea as above. */
	      case KEY:
//ScenSim-Port//		token = next_token (&val, (unsigned *)0, cfile);
//ScenSim-Port//		if (!parse_key (cfile)) {
//ScenSim-Port//			*lose = 1;
//ScenSim-Port//			return 0;
//ScenSim-Port//		}
//ScenSim-Port//		return 1;
...
//ScenSim-Port//int parse_zone (struct dns_zone *zone, struct parse *cfile)
//ScenSim-Port//{
//ScenSim-Port//	int token;
//ScenSim-Port//	const char *val;
//ScenSim-Port//	char *key_name;
//ScenSim-Port//	struct option_cache *oc;
//ScenSim-Port//	int done = 0;
//ScenSim-Port//
//ScenSim-Port//	token = next_token (&val, (unsigned *)0, cfile);
//ScenSim-Port//	if (token != LBRACE) {
//ScenSim-Port//		parse_warn (cfile, "expecting left brace");
//ScenSim-Port//		return 0;
//ScenSim-Port//	}
//ScenSim-Port//
//ScenSim-Port//	do {
//ScenSim-Port//	    token = peek_token (&val, (unsigned *)0, cfile);
//ScenSim-Port//	    switch (token) {
//ScenSim-Port//		  case PRIMARY:
//ScenSim-Port//		    if (zone -> primary) {
//ScenSim-Port//			    parse_warn (cfile,
//ScenSim-Port//					"more than one primary.");
//ScenSim-Port//			    skip_to_semi (cfile);
//ScenSim-Port//			    return 0;
//ScenSim-Port//		    }
//ScenSim-Port//		    if (!option_cache_allocate (&zone -> primary, MDL))
//ScenSim-Port//			    log_fatal ("can't allocate primary option cache.");
//ScenSim-Port//		    oc = zone -> primary;
//ScenSim-Port//		    goto consemup;
//ScenSim-Port//		    
//ScenSim-Port//		  case SECONDARY:
//ScenSim-Port//		    if (zone -> secondary) {
//ScenSim-Port//			    parse_warn (cfile, "more than one secondary.");
//ScenSim-Port//			skip_to_semi (cfile);
//ScenSim-Port//			return 0;
//ScenSim-Port//		    }
//ScenSim-Port//		    if (!option_cache_allocate (&zone -> secondary, MDL))
//ScenSim-Port//			    log_fatal ("can't allocate secondary.");
//ScenSim-Port//		    oc = zone -> secondary;
//ScenSim-Port//		  consemup:
//ScenSim-Port//		    token = next_token (&val, (unsigned *)0, cfile);
//ScenSim-Port//		    do {
//ScenSim-Port//			    struct expression *expr = (struct expression *)0;
//ScenSim-Port//			    if (!parse_ip_addr_or_hostname (&expr, cfile, 0)) {
//ScenSim-Port//				parse_warn (cfile,
//ScenSim-Port//					    "expecting IP addr or hostname.");
//ScenSim-Port//				skip_to_semi (cfile);
//ScenSim-Port//				return 0;
//ScenSim-Port//			    }
//ScenSim-Port//			    if (oc -> expression) {
//ScenSim-Port//				    struct expression *old =
//ScenSim-Port//					    (struct expression *)0;
//ScenSim-Port//				    expression_reference (&old,
//ScenSim-Port//							  oc -> expression,
//ScenSim-Port//							  MDL);
//ScenSim-Port//				    expression_dereference (&oc -> expression,
//ScenSim-Port//							    MDL);
//ScenSim-Port//				    if (!make_concat (&oc -> expression,
//ScenSim-Port//						      old, expr))
//ScenSim-Port//					log_fatal ("no memory for concat.");
//ScenSim-Port//				    expression_dereference (&expr, MDL);
//ScenSim-Port//				    expression_dereference (&old, MDL);
//ScenSim-Port//			    } else {
//ScenSim-Port//				    expression_reference (&oc -> expression,
//ScenSim-Port//							  expr, MDL);
//ScenSim-Port//				    expression_dereference (&expr, MDL);
//ScenSim-Port//			    }
//ScenSim-Port//			    token = next_token (&val, (unsigned *)0, cfile);
//ScenSim-Port//		    } while (token == COMMA);
//ScenSim-Port//		    if (token != SEMI) {
//ScenSim-Port//			    parse_warn (cfile, "expecting semicolon.");
//ScenSim-Port//			    skip_to_semi (cfile);
//ScenSim-Port//			    return 0;
//ScenSim-Port//		    }
//ScenSim-Port//		    break;
//ScenSim-Port//
//ScenSim-Port//	          case PRIMARY6:
//ScenSim-Port//		    if (zone->primary6) {
//ScenSim-Port//			    parse_warn(cfile, "more than one primary6.");
//ScenSim-Port//			    skip_to_semi(cfile);
//ScenSim-Port//			    return (0);
//ScenSim-Port//		    }
//ScenSim-Port//		    if (!option_cache_allocate (&zone->primary6, MDL))
//ScenSim-Port//			    log_fatal("can't allocate primary6 option cache.");
//ScenSim-Port//		    oc = zone->primary6;
//ScenSim-Port//		    goto consemup6;
//ScenSim-Port//
//ScenSim-Port//	          case SECONDARY6:
//ScenSim-Port//		    if (zone->secondary6) {
//ScenSim-Port//			    parse_warn(cfile, "more than one secondary6.");
//ScenSim-Port//			    skip_to_semi(cfile);
//ScenSim-Port//			    return (0);
//ScenSim-Port//		    }
//ScenSim-Port//		    if (!option_cache_allocate (&zone->secondary6, MDL))
//ScenSim-Port//			    log_fatal("can't allocate secondary6 "
//ScenSim-Port//				      "option cache.");
//ScenSim-Port//		    oc = zone->secondary6;
//ScenSim-Port//	          consemup6:
//ScenSim-Port//		    token = next_token(&val, NULL, cfile);
//ScenSim-Port//		    do {
//ScenSim-Port//			    struct expression *expr = NULL;
//ScenSim-Port//			    if (parse_ip6_addr_expr(&expr, cfile) == 0) {
//ScenSim-Port//				    parse_warn(cfile, "expecting IPv6 addr.");
//ScenSim-Port//				    skip_to_semi(cfile);
//ScenSim-Port//				    return (0);
//ScenSim-Port//			    }
//ScenSim-Port//			    if (oc->expression) {
//ScenSim-Port//				    struct expression *old = NULL;
//ScenSim-Port//				    expression_reference(&old, oc->expression,
//ScenSim-Port//							 MDL);
//ScenSim-Port//				    expression_dereference(&oc->expression,
//ScenSim-Port//							   MDL);
//ScenSim-Port//				    if (!make_concat(&oc->expression,
...
//ScenSim-Port//		    break;
//ScenSim-Port//
//ScenSim-Port//		  case KEY:
//ScenSim-Port//		    token = next_token (&val, (unsigned *)0, cfile);
//ScenSim-Port//		    token = peek_token (&val, (unsigned *)0, cfile);
//ScenSim-Port//		    if (token == STRING) {
//ScenSim-Port//			    token = next_token (&val, (unsigned *)0, cfile);
//ScenSim-Port//			    key_name = (char *)0;
//ScenSim-Port//		    } else {
//ScenSim-Port//			    key_name = parse_host_name (cfile);
//ScenSim-Port//			    if (!key_name) {
//ScenSim-Port//				    parse_warn (cfile, "expecting key name.");
//ScenSim-Port//				    skip_to_semi (cfile);
//ScenSim-Port//				    return 0;
//ScenSim-Port//			    }
//ScenSim-Port//			    val = key_name;
//ScenSim-Port//		    }
//ScenSim-Port//		    if (omapi_auth_key_lookup_name (&zone -> key, val) !=
//ScenSim-Port//			ISC_R_SUCCESS)
//ScenSim-Port//			    parse_warn (cfile, "unknown key %s", val);
//ScenSim-Port//		    if (key_name)
//ScenSim-Port//			    dfree (key_name, MDL);
//ScenSim-Port//		    if (!parse_semi (cfile))
//ScenSim-Port//			    return 0;
//ScenSim-Port//		    break;
//ScenSim-Port//		    
...
//ScenSim-Port//		    break;
//ScenSim-Port//	    }
//ScenSim-Port//	} while (!done);
//ScenSim-Port//
//ScenSim-Port//	token = next_token (&val, (unsigned *)0, cfile);
//ScenSim-Port//	if (token != RBRACE) {
//ScenSim-Port//		parse_warn (cfile, "expecting right brace.");
//ScenSim-Port//		return 0;
//ScenSim-Port//	}
//ScenSim-Port//	return 1;
//ScenSim-Port//}
...
//ScenSim-Port//int parse_key (struct parse *cfile)
//ScenSim-Port//{
//ScenSim-Port//	int token;
//ScenSim-Port//	const char *val;
//ScenSim-Port//	int done = 0;
//ScenSim-Port//	struct auth_key *key;
//ScenSim-Port//	struct data_string ds;
//ScenSim-Port//	isc_result_t status;
//ScenSim-Port//	char *s;
//ScenSim-Port//
//ScenSim-Port//	key = (struct auth_key *)0;
//ScenSim-Port//	if (omapi_auth_key_new (&key, MDL) != ISC_R_SUCCESS)
//ScenSim-Port//		log_fatal ("no memory for key");
//ScenSim-Port//
//ScenSim-Port//	token = peek_token (&val, (unsigned *)0, cfile);
//ScenSim-Port//	if (token == STRING) {
//ScenSim-Port//		token = next_token (&val, (unsigned *)0, cfile);
//ScenSim-Port//		key -> name = dmalloc (strlen (val) + 1, MDL);
//ScenSim-Port//		if (!key -> name)
//ScenSim-Port//			log_fatal ("no memory for key name.");
//ScenSim-Port//		strcpy (key -> name, val);
//ScenSim-Port//
//ScenSim-Port//	} else {
//ScenSim-Port//		key -> name = parse_host_name (cfile);
//ScenSim-Port//		if (!key -> name) {
//ScenSim-Port//			parse_warn (cfile, "expecting key name.");
//ScenSim-Port//			skip_to_semi (cfile);
//ScenSim-Port//			goto bad;
//ScenSim-Port//		}
//ScenSim-Port//	}
//ScenSim-Port//
//ScenSim-Port//	token = next_token (&val, (unsigned *)0, cfile);
...
//ScenSim-Port//		token = next_token (&val, (unsigned *)0, cfile);
//ScenSim-Port//		switch (token) {
//ScenSim-Port//		      case ALGORITHM:
//ScenSim-Port//			if (key -> algorithm) {
//ScenSim-Port//				parse_warn (cfile,
//ScenSim-Port//					    "key %s: too many algorithms",
//ScenSim-Port//					    key -> name);
//ScenSim-Port//				goto rbad;
//ScenSim-Port//			}
//ScenSim-Port//			key -> algorithm = parse_host_name (cfile);
//ScenSim-Port//			if (!key -> algorithm) {
//ScenSim-Port//				parse_warn (cfile,
//ScenSim-Port//					    "expecting key algorithm name.");
//ScenSim-Port//				goto rbad;
//ScenSim-Port//			}
//ScenSim-Port//			if (!parse_semi (cfile))
//ScenSim-Port//				goto rbad;
//ScenSim-Port//			/* If the algorithm name isn't an FQDN, tack on
//ScenSim-Port//			   the .SIG-ALG.REG.NET. domain. */
//ScenSim-Port//			s = strrchr (key -> algorithm, '.');
//ScenSim-Port//			if (!s) {
//ScenSim-Port//			    static char add [] = ".SIG-ALG.REG.INT.";
//ScenSim-Port//			    s = dmalloc (strlen (key -> algorithm) +
//ScenSim-Port//					 sizeof (add), MDL);
//ScenSim-Port//			    if (!s) {
//ScenSim-Port//				log_error ("no memory for key %s.",
//ScenSim-Port//					   "algorithm");
//ScenSim-Port//				goto rbad;
//ScenSim-Port//			    }
//ScenSim-Port//			    strcpy (s, key -> algorithm);
//ScenSim-Port//			    strcat (s, add);
//ScenSim-Port//			    dfree (key -> algorithm, MDL);
//ScenSim-Port//			    key -> algorithm = s;
//ScenSim-Port//			} else if (s [1]) {
//ScenSim-Port//			    /* If there is no trailing '.', hack one in. */
//ScenSim-Port//			    s = dmalloc (strlen (key -> algorithm) + 2, MDL);
//ScenSim-Port//			    if (!s) {
//ScenSim-Port//				    log_error ("no memory for key %s.",
//ScenSim-Port//					       key -> algorithm);
//ScenSim-Port//				    goto rbad;
//ScenSim-Port//			    }
//ScenSim-Port//			    strcpy (s, key -> algorithm);
//ScenSim-Port//			    strcat (s, ".");
//ScenSim-Port//			    dfree (key -> algorithm, MDL);
//ScenSim-Port//			    key -> algorithm = s;
//ScenSim-Port//			}
//ScenSim-Port//			break;
//ScenSim-Port//
//ScenSim-Port//		      case SECRET:
//ScenSim-Port//			if (key -> key) {
//ScenSim-Port//				parse_warn (cfile, "key %s: too many secrets",
//ScenSim-Port//					    key -> name);
//ScenSim-Port//				goto rbad;
//ScenSim-Port//			}
//ScenSim-Port//
//ScenSim-Port//			memset (&ds, 0, sizeof(ds));
//ScenSim-Port//			if (!parse_base64 (&ds, cfile))
//ScenSim-Port//				goto rbad;
//ScenSim-Port//			status = omapi_data_string_new (&key -> key, ds.len,
//ScenSim-Port//							MDL);
//ScenSim-Port//			if (status != ISC_R_SUCCESS)
//ScenSim-Port//				goto rbad;
//ScenSim-Port//			memcpy (key -> key -> value,
//ScenSim-Port//				ds.buffer -> data, ds.len);
//ScenSim-Port//			data_string_forget (&ds, MDL);
//ScenSim-Port//
//ScenSim-Port//			if (!parse_semi (cfile))
//ScenSim-Port//				goto rbad;
//ScenSim-Port//			break;
...
//ScenSim-Port//		goto rbad;
//ScenSim-Port//	}
//ScenSim-Port//	/* Allow the BIND 8 syntax, which has a semicolon after each
//ScenSim-Port//	   closing brace. */
//ScenSim-Port//	token = peek_token (&val, (unsigned *)0, cfile);
//ScenSim-Port//	if (token == SEMI)
//ScenSim-Port//		token = next_token (&val, (unsigned *)0, cfile);
//ScenSim-Port//
//ScenSim-Port//	/* Remember the key. */
//ScenSim-Port//	status = omapi_auth_key_enter (key);
//ScenSim-Port//	if (status != ISC_R_SUCCESS) {
//ScenSim-Port//		parse_warn (cfile, "tsig key %s: %s",
//ScenSim-Port//			    key -> name, isc_result_totext (status));
//ScenSim-Port//		goto bad;
//ScenSim-Port//	}
//ScenSim-Port//	omapi_auth_key_dereference (&key, MDL);
//ScenSim-Port//	return 1;
//ScenSim-Port//
//ScenSim-Port//      rbad:
//ScenSim-Port//	skip_to_rbrace (cfile, 1);
//ScenSim-Port//      bad:
//ScenSim-Port//	omapi_auth_key_dereference (&key, MDL);
//ScenSim-Port//	return 0;
//ScenSim-Port//}
...
//ScenSim-Port//int parse_on_statement (result, cfile, lose)
//ScenSim-Port//	struct executable_statement **result;
//ScenSim-Port//	struct parse *cfile;
//ScenSim-Port//	int *lose;
int parse_on_statement (struct executable_statement **result, struct parse *cfile, int *lose)//ScenSim-Port//
{
	enum dhcp_token token;
	const char *val;
...
//ScenSim-Port//int parse_switch_statement (result, cfile, lose)
//ScenSim-Port//	struct executable_statement **result;
//ScenSim-Port//	struct parse *cfile;
//ScenSim-Port//	int *lose;
int parse_switch_statement (struct executable_statement **result, struct parse *cfile, int *lose)//ScenSim-Port//
{
	enum dhcp_token token;
	const char *val;
...
//ScenSim-Port//int parse_case_statement (result, cfile, lose, case_context)
//ScenSim-Port//	struct executable_statement **result;
//ScenSim-Port//	struct parse *cfile;
//ScenSim-Port//	int *lose;
//ScenSim-Port//	enum expression_context case_context;
int parse_case_statement (struct executable_statement **result, struct parse *cfile, int *lose, enum expression_context case_context)//ScenSim-Port//
{
	enum dhcp_token token;
	const char *val;
...
//ScenSim-Port//int parse_if_statement (result, cfile, lose)
//ScenSim-Port//	struct executable_statement **result;
//ScenSim-Port//	struct parse *cfile;
//ScenSim-Port//	int *lose;
int parse_if_statement (struct executable_statement **result, struct parse *cfile, int *lose)//ScenSim-Port//
{
	enum dhcp_token token;
	const char *val;
	int parenp;
...
//ScenSim-Port//int parse_boolean_expression (expr, cfile, lose)
//ScenSim-Port//	struct expression **expr;
//ScenSim-Port//	struct parse *cfile;
//ScenSim-Port//	int *lose;
int parse_boolean_expression (struct expression **expr, struct parse *cfile, int *lose)//ScenSim-Port//
{
	/* Parse an expression... */
...
//ScenSim-Port//int parse_boolean (cfile)
//ScenSim-Port//	struct parse *cfile;
int parse_boolean (struct parse *cfile)//ScenSim-Port//
{
	const char *val;
	int rv;
...
//ScenSim-Port//int parse_data_expression (expr, cfile, lose)
//ScenSim-Port//	struct expression **expr;
//ScenSim-Port//	struct parse *cfile;
//ScenSim-Port//	int *lose;
int parse_data_expression (struct expression **expr, struct parse *cfile, int *lose)//ScenSim-Port//
{
	/* Parse an expression... */
...
//ScenSim-Port//int parse_numeric_expression (expr, cfile, lose)
//ScenSim-Port//	struct expression **expr;
//ScenSim-Port//	struct parse *cfile;
//ScenSim-Port//	int *lose;
int parse_numeric_expression (struct expression **expr, struct parse *cfile, int *lose)//ScenSim-Port//
{
	/* Parse an expression... */
...
//ScenSim-Port//int parse_dns_expression (expr, cfile, lose)
//ScenSim-Port//	struct expression **expr;
//ScenSim-Port//	struct parse *cfile;
//ScenSim-Port//	int *lose;
int parse_dns_expression (struct expression **expr, struct parse *cfile, int *lose)//ScenSim-Port//
{
	/* Parse an expression... */
...
//ScenSim-Port//int parse_non_binary (expr, cfile, lose, context)
//ScenSim-Port//	struct expression **expr;
//ScenSim-Port//	struct parse *cfile;
//ScenSim-Port//	int *lose;
//ScenSim-Port//	enum expression_context context;
int parse_non_binary (struct expression **expr, struct parse *cfile, int *lose, enum expression_context context)//ScenSim-Port//
{
	enum dhcp_token token;
	const char *val;
	...
	switch (token) {
		...
		if (!expression_allocate (expr, MDL))
			log_fatal ("can't allocate expression");
		(*expr) -> op = expr_not;
//ScenSim-Port//		if (!parse_non_binary (&(*expr) -> data.not,
		if (!parse_non_binary (&(*expr) -> data.not_,//ScenSim-Port//
...
//ScenSim-Port//		if (!is_boolean_expression ((*expr) -> data.not)) {
		if (!is_boolean_expression ((*expr) -> data.not_)) {//ScenSim-Port//
			*lose = 1;
			parse_warn (cfile, "boolean expression expected");
			skip_to_semi (cfile);
			expression_dereference (expr, MDL);
		...
		s = (opcode == expr_ns_add
		     ? "old-dns-update"
		     : "old-dns-delete");
//ScenSim-Port//		cptr = dmalloc (strlen (s) + 1, MDL);
		cptr = dmalloc (strlen (s) + 1, MDL);//ScenSim-Port//
		if (!cptr)
		...
		(*expr) -> op = expr_variable_reference;
		(*expr) -> data.variable =
//ScenSim-Port//			dmalloc (strlen (s) + 1, MDL);
			dmalloc (strlen (s) + 1, MDL);//ScenSim-Port//
...
//ScenSim-Port//		(*expr) -> data.variable = dmalloc (strlen (val) + 1, MDL);
		(*expr) -> data.variable = (char*)dmalloc (strlen (val) + 1, MDL);//ScenSim-Port//
		if (!(*expr)->data.variable)
			log_fatal ("can't allocate variable name");
...
//ScenSim-Port//		cptr = dmalloc (strlen (val) + 1, MDL);
		cptr = (char*)dmalloc (strlen (val) + 1, MDL);//ScenSim-Port//
		if (!cptr)
			log_fatal ("can't allocate variable name");
		strcpy (cptr, val);
...
//ScenSim-Port//int parse_expression (expr, cfile, lose, context, plhs, binop)
//ScenSim-Port//	struct expression **expr;
//ScenSim-Port//	struct parse *cfile;
//ScenSim-Port//	int *lose;
//ScenSim-Port//	enum expression_context context;
//ScenSim-Port//	struct expression **plhs;
//ScenSim-Port//	enum expr_op binop;
int parse_expression (struct expression **expr, struct parse *cfile, int *lose, enum expression_context context, struct expression **plhs, enum expr_op binop)//ScenSim-Port//
{
	enum dhcp_token token;
	const char *val;
...
//ScenSim-Port//int parse_option_data (expr, cfile, lookups, option)
//ScenSim-Port//struct expression **expr;
//ScenSim-Port//struct parse *cfile;
//ScenSim-Port//int lookups;
//ScenSim-Port//struct option *option;
int parse_option_data (struct expression **expr, struct parse *cfile, int lookups, struct option *option)//ScenSim-Port//
{
	const char *val;
	const char *fmt = NULL;
...
//ScenSim-Port//int parse_option_statement (result, cfile, lookups, option, op)
//ScenSim-Port//	struct executable_statement **result;
//ScenSim-Port//	struct parse *cfile;
//ScenSim-Port//	int lookups;
//ScenSim-Port//	struct option *option;
//ScenSim-Port//	enum statement_op op;
int parse_option_statement (struct executable_statement **result, struct parse *cfile, int lookups, struct option *option, enum statement_op op)//ScenSim-Port//
{
	const char *val;
	enum dhcp_token token;
...
//ScenSim-Port//int parse_option_token (rv, cfile, fmt, expr, uniform, lookups)
//ScenSim-Port//	struct expression **rv;
//ScenSim-Port//	struct parse *cfile;
//ScenSim-Port//	const char **fmt;
//ScenSim-Port//	struct expression *expr;
//ScenSim-Port//	int uniform;
//ScenSim-Port//	int lookups;
int parse_option_token (struct expression **rv, struct parse *cfile, const char **fmt, struct expression *expr, int uniform, int lookups)//ScenSim-Port//
{
	const char *val;
	enum dhcp_token token;
...
//ScenSim-Port//int parse_option_decl (oc, cfile)
//ScenSim-Port//	struct option_cache **oc;
//ScenSim-Port//	struct parse *cfile;
int parse_option_decl (struct option_cache **oc, struct parse *cfile)//ScenSim-Port//
{
	const char *val;
	int token;
	u_int8_t buf [4];
...
//ScenSim-Port// bp->data is always true
//ScenSim-Port//	if (!bp -> data)
//ScenSim-Port//		log_fatal ("out of memory allocating option data.");
...
//ScenSim-Port//int parse_X (cfile, buf, max)
//ScenSim-Port//	struct parse *cfile;
//ScenSim-Port//	u_int8_t *buf;
//ScenSim-Port//	unsigned max;
int parse_X (struct parse *cfile, u_int8_t *buf, unsigned max)//ScenSim-Port//
{
	int token;
	const char *val;
	unsigned len;
...
//ScenSim-Port//	syslog (log_priority | LOG_ERR, "%s", cfile -> token_line);
	syslog ((log_priority | LOG_ERR), "%s", mbuf); //ScenSim-Port//
	syslog ((log_priority | LOG_ERR), "%s", cfile -> token_line); //ScenSim-Port//
	if (cfile -> lexchar < 81)
//ScenSim-Port//		syslog (log_priority | LOG_ERR, "%s^", lexbuf);
		syslog ((log_priority | LOG_ERR), "%s^", lexbuf); //ScenSim-Port//
//ScenSim-Port//#endif
...
//ScenSim-Port//		IGNORE_RET (write (STDERR_FILENO, cfile -> token_line,
//ScenSim-Port//				   strlen (cfile -> token_line)));
//ScenSim-Port//		IGNORE_RET (write (STDERR_FILENO, "\n", 1));
//ScenSim-Port//		if (cfile -> lexchar < 81)
//ScenSim-Port//			IGNORE_RET (write (STDERR_FILENO, lexbuf, lix));
//ScenSim-Port//		IGNORE_RET (write (STDERR_FILENO, "^\n", 2));
//ScenSim-Port//	}
